 ...
 *
 * <p>Pass in a valid JSON document string to {@link JsonHiveSchema#createHiveSchema} and it will
 * return a Hive schema for the JSON document.
 *
 * <p>It supports embedded JSON objects, arrays and the standard JSON scalar types: strings,
 * numbers, booleans and null. You probably don't want null in the JSON document you provide as Hive
 * can't use that. For numbers - if the example value has a decimal, it will be typed as "double".
 * If the number has no decimal, it will be typed as "int".
 *
 * <p>This program uses the JSON parsing code from json.org and that code is included in this
 * library, since it has not been packaged and made available for maven/ivy/gradle dependency
 * resolution.
 *
 * <p><strong>Use of main method:</strong> <br>
 * JsonHiveSchema has a main method that takes a file path to a JSON doc - this file should have
 * only one JSON file in it. An optional second argument can be provided to name the Hive table that
 * is generated.
 */
public class JsonHiveSchema {
   ...
   *
   * <p>The Hive schema columns will be printed in alphabetical order - overall and within
   * subsections.
   *
   * @param json
